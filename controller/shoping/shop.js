...
const Rating = require('../../models/ugc/rating')
...
class Shopping extends AddressComponent {
    ...
    async getRestaurants(req, res) {
        ...
        let filter = {}
        // 获取对应分类
        if (restaurant_category_ids && restaurant_category_ids.length) {
            const category = categoryHandle.getCategoryById(restaurant_category_ids[0])
    ...
    async getRestaurantDetail(req, res) {
        ...
        try {
            const restaurant = await ShopModel.findOne({
                id: restaurant_id,
            }, '-_id');
            if (!restaurant) {
                return res.send({
    ...
    async addRestaurant(req, res) {
        ...
        //商店支持的活动
        if (fields.activities && fields.activities.length) {
